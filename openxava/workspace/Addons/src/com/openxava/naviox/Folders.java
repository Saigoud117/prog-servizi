	...
	private List<MetaModule> getFolderModules(Folder folder) { 
		...
		for (Module module: folderModules) {
			...
			try { 
				...
				if (!includeFixedAndBoookmarkModules && this.modules.getFixedModules().contains(metaModule)) continue; 
				if (!includeFixedAndBoookmarkModules && this.modules.getBookmarkModules().contains(metaModule)) continue;				
