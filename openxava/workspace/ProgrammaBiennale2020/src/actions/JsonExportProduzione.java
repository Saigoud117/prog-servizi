...
public class JsonExportProduzione extends ViewBaseAction  {
		...
		*/
		...
		try {
			...
			*/
			...
			PubblicazioneResponse response =  RetrofitPubblicaFornitureServizi(loginGbj.getToken(), "3", p);
			//PubblicaFornitureServizi(loginGbj.getToken(), json, "2");
			
			EsportazioneDati e = new EsportazioneDati();
		    Date date = new Date();
		    e.setData(date);
		    e.setProduzione(true);
	...
	private PubblicazioneResponse RetrofitPubblicaFornitureServizi(String token, String modalitaInvio, PubblicazioneFornitureServizi p) throws IOException {
		ApiService service = WebClient.retrofitProduzione().create(ApiService.class);
		Call<PubblicazioneResponse> repos = service.PubblicaFornitureServiziProduzione(token, modalitaInvio, p);
	...
	private Login RetrofitLogin() throws IOException {
		...
		String username = ProgrammaBiennalePreferences.getDefaultUsernameProduzione();
		String password = ProgrammaBiennalePreferences.getDefaultPasswordProduzione();
		String clientId = ProgrammaBiennalePreferences.getDefaultClientIdProduzione();
		String clientKey = ProgrammaBiennalePreferences.getDefaultClientKeyProduzione();
		
		ApiService service = WebClient.retrofitProduzione().create(ApiService.class);
		Call<Login> repos = service.WsLoginProduzione(username, password, clientId, clientKey);
